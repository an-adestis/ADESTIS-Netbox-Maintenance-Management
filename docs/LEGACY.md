...
[Click here](https://pypi.org/project/adestis-netbox-maintenance/)
...
$ cd adestis-netbox-maintenance
$ docker build -f Dev-Dockerfile -t adestis-netbox-maintenance-plugin .
```
...
services:
  netbox: &netbox
    image: adestis-netbox-maintenance-management-plugin:latest
```
...
```python
PLUGINS = ['adestis_netbox_maintenance_management']
```
...
Mount the migrations folder, start the infrastructure services and run `docker-compose run netbox sh -c "python manage.py makemigrations adestis_netbox_maintenance_management"`. After that unmount the migrations folder and rebuild the docker container
